...
const Header = () => {
  ...
  const [selectedCategorySlug, setSelectedCategorySlug] = useState("0");
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim() === "" && selectedCategorySlug !== "0") {
      router.push(`/shop-without-sidebar?${selectedCategorySlug}`);
      return;
    }
  ...
  const handleCategorySelect = (option: { label: string; value: string }) => {
    ...
    if (option.value !== "0") {
      router.push(`/shop-without-sidebar?${option.value}`);
    }
  };
          ...
          <div className="xl:w-auto flex-col sm:flex-row w-full flex sm:justify-between sm:items-center gap-5 sm:gap-10">
            <Link className="flex-shrink-0 flex items-center gap-3" href="/">
              <Image
                ...
                width={36}
                height={36}
                className="h-9 w-auto object-cover rounded-full overflow-hidden"
              />
              <span className="font-bold text-xl text-dark hidden sm:block">
                  {settings['store_name'] || "Nextmerce"}
              </span>
            </Link>
            ...
            <div className="hidden xl:flex items-center gap-3.5">
              ...
              <div>
                ...
                <p className="font-medium text-custom-sm text-dark">
                  {settings['support_phone'] || "(+965) 7492-3477"}
                </p>
              </div>
            </div>
              ...
              <div className="flex items-center gap-5">
                ...
                {user ? (
                   <div className="relative group">
                     ...
                     >
                        <div className="flex items-center justify-center rounded-full overflow-hidden w-10 h-10 border border-gray-3 bg-gray-1 text-blue font-bold text-lg hover:border-blue transition-colors">
                           {user.avatarUrl ? (
                               <Image 
                                    src={user.avatarUrl} 
                        ...
                        <div className="hidden sm:block text-left">
                            <span className="block text-2xs text-dark-4 uppercase leading-none">
                                {user.roleCode === 'ADMIN' ? 'Quản Trị' : 'Tài Khoản'}
                            </span>
                     ...
                     {/* User Dropdown */}
                     <div className={`absolute right-0 top-full mt-2 w-48 bg-white border border-gray-3 rounded-md shadow-lg py-2 z-99999 transition-all duration-200 ${userDropdown ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
                        <Link 
                            href="/my-account" 
                            className="block px-4 py-2 text-sm text-dark hover:bg-gray-1 hover:text-blue"
                        ...
                        <button 
                            ...
                            className="w-full text-left px-4 py-2 text-sm text-red hover:bg-gray-1"
                        >
                            Đăng Xuất
                        </button>
                     </div>

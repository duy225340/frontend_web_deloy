...
const mapToProduct = (item: ProductApiResponse): Product => {
  ...
  const isReviewsArray = Array.isArray(item.reviews);
  const reviewsCount = isReviewsArray 
    ? (item.reviews as any[]).length 
    : (item.totalReviews || item.reviewsCount || (typeof item.reviews === 'number' ? item.reviews : 0));

  const averageRating = item.averageRating || item.rating || (isReviewsArray && reviewsCount > 0
    ? (item.reviews as any[]).reduce((acc: number, r: any) => acc + (r.rating || 0), 0) / reviewsCount
    : 0);
...
const ShopWithSidebar = () => {
  ...
  const [categories, setCategories] = useState<{ name: string; slug: string; id: number; products: number; isRefined: boolean }[]>([]);
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      setProducts(data.content.map(mapToProduct));
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setCategories(
          catData.filter((c) => c.isActive).map((cat: Category) => ({
            name: cat.title,
            slug: cat.slug,
            id: cat.id,
            products: cat.productCount || 0,
            isRefined: selectedCategorySlug === cat.slug,
          }))
        );
  ...
  useEffect(() => {
    fetchProducts();
  }, [debouncedSearchQuery, selectedCategorySlug, minPrice, maxPrice, currentPage, sortBy, selectedAttributes]);

  // Update filters  // Sync URL parameters
  useEffect(() => {
    let currentSlug = searchParams.get("categorySlug");
    const search = searchParams.get("search");

    if (!currentSlug) {
      const urlParams = window.location.search;
      if (urlParams) {
        const potentialSlug = urlParams.substring(1).split('&')[0];
        if (potentialSlug && !potentialSlug.includes('=')) {
          currentSlug = potentialSlug;
        }
      }
    }

    if (search) setSearchQuery(search);
    if (currentSlug) setSelectedCategorySlug(currentSlug);
  }, [searchParams]);
  ...
  const handleCategorySelect = (categorySlug: string | null) => {
    ...
    setCurrentPage(0);
    // Update URL to match new parameter-less format ?category-slug
    if (categorySlug) {
      window.history.pushState(null, '', `/shop-with-sidebar?${categorySlug}`);
    } else {
      window.history.pushState(null, '', '/shop-with-sidebar');
    }
  };
  ...
  const handleSortChange = (option: { label: string; value: string }) => {
    const sortMap: { [key: string]: string } = {
      "0": "createdAt,desc",
      "1": "createdAt,desc", // Best Selling logic to be improved
      "2": "createdAt,asc",
    };
    setSortBy(sortMap[option.value] || "createdAt,desc");
    setCurrentPage(0);
  };
  ...
  return (
    <>
      <Breadcrumb
        title={searchQuery ? `Tìm kiếm: "${searchQuery}"` : "Khám Phá Tất Cả Sản Phẩm"}
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div className="bg-white shadow-1 rounded-lg py-4 px-5">
                    <div className="flex items-center justify-between">
                      <p>Bộ Lọc:</p>
                      <button type="button" onClick={handleClearFilters} className="text-blue">
                        Xóa Tất Cả
                      </button>
                    </div>
                  </div>
                  ...
                  {/* Search box */}
                  <div className="bg-white shadow-1 rounded-lg py-4 px-5">
                    <p className="font-medium text-dark mb-3">Tìm Kiếm</p>
                    <input
                      type="text"
                      value={searchQuery}
                      onChange={(e) => setSearchQuery(e.target.value)}
                      placeholder="Tìm kiếm sản phẩm..."
                      className="w-full border border-gray-3 rounded-md py-2 px-3 outline-none focus:border-blue"
                    />
                  </div>
              ...
              <div className="rounded-lg bg-white shadow-1 pl-3 pr-2.5 py-2.5 mb-6">
                <div className="flex items-center justify-between">
                  <div className="flex flex-wrap items-center gap-4">
                    <CustomSelect options={options} onSelect={handleSortChange} />
                    <p>
                      Hiển thị <span className="text-dark">{products.length} trên {totalProducts}</span> sản phẩm
                    </p>
                  </div>
                ...
                <div className="flex justify-center mt-15">
                  <div className="bg-white shadow-1 rounded-md p-2">
                    <ul className="flex items-center gap-1">
                      ...
                      {totalPages > 5 && <li className="px-2">...</li>}
                      <li>
                        <button
                          type="button"

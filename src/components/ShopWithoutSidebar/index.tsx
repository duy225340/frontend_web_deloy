...
const ShopWithoutSidebar = () => {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      let currentSlug = urlCategorySlug;
      // Get slug from standalone query param (e.g. ?ao-nam)
      if (!currentSlug) {
      ...
      if (currentSlug) {
        params.set("categorySlug", currentSlug);
      }
      if (urlSearch) {
      ...
      // If filtering by category, try to get category name
      if (currentSlug && data.content.length > 0 && data.content[0].categoryName) {
  ...
  useEffect(() => {
    ...
    setCurrentPage(0);
    // Tự động gọi fetchProducts trong useEffect này cũng sẽ trigger, nma phụ thuộc vào search params và window.location.search
    // Để an toàn, chúng ta sẽ để fetchProducts tự tính toán currentSlug mỗi lần trigger.
  }, [urlCategorySlug, urlSearch, searchParams]);

  useEffect(() => {
    fetchProducts();
  }, [currentPage, sortBy, urlCategorySlug, urlSearch, searchParams]);

  const handleSortChange = (option: { label: string; value: string }) => {
    const sortMap: { [key: string]: string } = {
      "0": "createdAt,desc",
      "1": "createdAt,desc", // Placeholder for best selling
      "2": "createdAt,asc",
    };
  ...
  const getPageTitle = () => {
    if (urlSearch) return `Kết quả tìm kiếm: "${urlSearch}"`;
                  ...
                  <div className="bg-white shadow-1 rounded-md p-2">
                    <ul className="flex items-center gap-1">
                      ...
                      {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
                         // Simple pagination logic for display, can be enhanced
                         let pageNum = i;
                      ...
                      {totalPages > 5 && currentPage < totalPages - 3 && <li className="px-2">...</li>}
